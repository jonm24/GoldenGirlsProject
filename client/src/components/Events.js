...
function CreateEventForm({ addEvent, closeModal, setName, setLocation, setTime, setDescription }) {
  return (
    <Fragment>
      <button style={{ position: "absolute", right: "10px", top: "10px", cursor: "pointer" }} onClick={closeModal}>
        close
      </button>
      <form
...
function Card({ elem }) {
  ...
  return (
    <div className="card flex column">
      ...
      <div
        className="flex"
        style={{ alignSelf: "flex-end", width: "100%", alignItems: "baseline", justifyContent: "space-between" }}
      >
        <h2>{cleanLocale}</h2>
        {!attending ? (
          <div className="unsure flex">
            <h3 onClick={() => setEvent("Yes")} className="yes">
              ✅
            </h3>
            <h3 onClick={() => setEvent("Maybe")} style={{ margin: "0px 12px" }} className="maybe">
              ❓
            </h3>
            <h3 onClick={() => setEvent("No")} className="no">
              ❌
            </h3>
          </div>
        ) : attending === "Yes" ? (
